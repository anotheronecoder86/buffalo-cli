...
import (
	...
	"buffalo-cli/internal/genny/newapp/web"
	"buffalo-cli/internal/genny/testrunner"
...
func Test_New(t *testing.T) {
	...
	cmds := []string{
		"go mod tidy",
		"go build -tags bar -o bin/foo " + appBinaryFolder(),
		"go mod tidy",
	}
...
func Test_NewWithoutBuildDeps(t *testing.T) {
	envy.Temp(func() {
		...
		cmds := []string{"go mod tidy", "go build -tags bar -o bin/foo " + appBinaryFolder()}
		r.Len(res.Commands, len(cmds))
		for i, c := range res.Commands {
			eq(r, cmds[i], c)
		}
